...
class BurgerBuilder extends Component {
  ...
  continueCheckoutHandler = async () => {
    ...
    for (let i in ingredients) {
      const uriComponent = `${encodeURIComponent(i)}=${encodeURIComponent(
        ingredients[i],
      )}`;
      queryParams.push(uriComponent);
    }
    queryParams.push(
      `${encodeURIComponent('totalPrice')}=${this.state.totalPrice}`,
    );
  ...
  updatePurchaseable = ingredients => {
    ...
    const sum = Object.values(ingredients).reduce((total, next) => {
  ...
  render() {
    ...
    let burgerBuilder = this.state.error ? (
      <p>The ingedients couldn't be loaded so the app is broke :-(</p>
    ) : (
      <Spinner />
    );
